...
*   **`verne_durand.py`**: A Python-based autonomous harness that executes a plan (list of tasks) by creating Jules sessions, monitoring them, and merging the results. Uses the `jules-agent-sdk`.
...
### 1. Verne Durand (Autonomous Harness)
...
uv run verne_durand.py --plan PLAN.md --project /path/to/target/repo
```
...
*   `verne_durand.py`: Main automation script.
